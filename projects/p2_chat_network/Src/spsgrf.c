...
void SPSGRF_Init(void)
{
  ...
  xStackInit.xControlLength 	= CONTROL_LENGTH;			// Not using Control bits
  ...
  xStackAddress.cMyAddress 					= MY_ADDRESS;						// CHANGED TO MY ADDRESS
  ...
  xStackLlp.xPiggybacking	= S_DISABLE;
  xStackLlp.xNMaxRetx		= 0;
...
void SPSGRF_StartTx(uint8_t *txBuff, uint8_t txLen)
{
  ...
  // exit Tx or Rx and go to Ready State
//  g_xStatus.MC_STATE;
