...
void initButtonInterrupt(void) {
	...
	NVIC_SetPriority(EXTI15_10_IRQn, 5);					// set interrupt priority
...
void EXTI15_10_IRQHandler(void) {
	...
	if(EXTI->PR1 & EXTI_PR1_PIF13) {
		RCC->APB1ENR1 |= RCC_APB1ENR1_TIM2EN;		// turn on clock to TIM2 (for motors)
		xSemaphoreGiveFromISR( getSkittleSema, &xHigherPriorityTaskWoken );
