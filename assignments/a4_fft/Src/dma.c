...
void init_P2M_DMA(volatile uint32_t *peripheral, uint32_t *dest) {
	...
	DMA1_Channel1->CCR |= ((0x1 << 10) 	// 32-bit Memory data transfers   0x2 is 32-bit
...
//	);
	...
	DMA1_Channel1->CNDTR = 2048;		// number of data transfers
...
//	__enable_irq();
//
	DMA1_Channel1->CCR |= (0x1 << 0);	// enable DMA channel for data transfers
