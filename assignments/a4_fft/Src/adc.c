...
//	ADC1->CFGR &= ~(0x7 << 17);				// 1 channel Discont Mode
//	ADC1->CFGR |=  (0x1 << 16);				// Enable Discontinuous Mode
...
//	ADC1->SMPR1 |=  (0x3 << 15);			// set the sampling rate for channel 5
...
//	ADC1->CFGR &= ~(0xF << 6);				// EXTEN SEL cleared
	ADC1->CFGR |=  (0x3 << 6);				// EXTEN SEL set to TIM2 CH2
//	ADC1->CFGR &= ~(0x3 << 10);				// Clear EXTEN[1:0]
	ADC1->CFGR |=  (0x1 << 10);				// EXTEN[1:0] = 1; Rising Edge Trigger
